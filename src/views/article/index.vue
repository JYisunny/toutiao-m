<template>
  <div class="article-container">
    ...
    <div class="main-wrap">
      ...
      <div v-else-if="article.title" class="article-detail">
        ...
        <van-cell class="user-info" center :border="false">
          ...
          <!--
            模板中的 $event 是事件参数(模板传的参:!this.article.is_followed)
            当我们传递给子组件的数据，既要用 还要改
              传递: props
    ...
    <!-- 评论回复 -->
    <!-- 弹出层是懒渲染：只有在第一次展示的时候，才会渲染里面的内容，之后它的关闭和显示都是在切换内容的显示和隐藏 -->
      <van-popup
        v-model="isReplyShow"
        position="bottom"
        style="height: 100%;"
...
export default {
  ...
  methods: {
    async loadArticle () {
      this.loading = true // 展示loading 加载中
      try {
        const { data } = await getArticleById(this.articleId)
        /* if (Math.random() > 0.5) { // 测试出现错误情况
          JSON.parse('sadsadsads')
        } */
        ...
        // 初始化图片点击预览
        // console.log(this.$refs['article-content']) // 数据驱动视图 不是立即的,所以拿不到DOM，可以利用定时器
        setTimeout(() => {
          this.previewImage()
        }, 0)
    ...
    previewImage () {
      ...
      const images = [] // 获取所有 img 地址
      imgs.forEach((img, index) => {
        images.push(img.src)
        img.onclick = () => {
          // 每个img 注册点击事件，在处理函数中调用预览
          ImagePreview({
            images,
